...
exports.deleteTaxRule = async(id) => {
    const taxRule =JSON.parse(JSON.stringify(await db.Tax_Rules.findOne({where:{
        id
    }})));
    ...
    if(orders.length>0){
        throw new HTTPError('Tax Rule cannot be deleted since there are orders including it',400);
    }
...
exports.updateTaxRule = async(data,id) =>{
    const taxRule = await db.Tax_Rules.findOne({where:{
        id
    }});
    if(!taxRule) throw new NotFoundError('Tax Rule With Given Id is Not Found');
    await db.Tax_Rules.update({...data},{
        where:{id}
    });
    return {
